-- OPD-41 (TEST): Add contest-end emoji to manager critical notification copy
...
BEGIN
  ...
  _notify AS (
    ...
    SELECT
      ...
      format(
        '⏱️ Twój konkurs %s dobiegł końca! Liczba zebranych ofert: %s. Kliknij, aby otworzyć bezpieczną tabelę porównawczą i wygenerować protokół.',
        su.title,
        su.offers_count
      ),
      jsonb_build_object(
